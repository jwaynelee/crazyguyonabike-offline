...
abstract class DictionaryBase {
	 ...
	 * 
	 * @param word
	 *            the word to check. Can't be null.
	...
	public boolean exist(CharacterIterator word) {
		idx = 0;
		for (int i = word.getBeginIndex(); i< word.getEndIndex();) {
			char c = word.next();
			while (idx < size && tree[idx] < c) {
				idx += 3;
			}
			...
			if (i == word.getEndIndex() - 1 && isWordMatch()) {
				return true;
			}
			idx = readIndex();
			if (idx <= 0)
	 ...
	 * 
	 * @param word
	 *            the wrong spelled word. Can't be null.
	...
	/**
	 * Es wird nach verschiedenen Regeln nach �hnlichen W�rtern gesucht. Je
	 * nach Regel gibt es einen anderen diff. Jekleiner der diff desto
	 * �hnlicher. Diese Methode ruft sich rekursiv auf.
	 * 
	 * @param list
	 *            Kontainer f�r die gefundenen W�rter
	 * @param chars
	 *            bis zur charPosition bereits gemappte Buchstaben, danach noch
	 *            zu mappende des orignal Wortes
	 * @param charPosition
	 *            Zeichenposition im char array
	 * @param lastIdx
	 *            Position im Suchindex der zur aktuellen Zeichenposition zeigt.
	 * @param diff
	 *            Die Un�hnlichkeit bis zur aktuellen Zeichenposition
	 */
	 ...
	 * variable <code>idx</code> point to the location. If not found then it
	 * point on the next character (char value) item in the node.
	 * 
	 * @param c
